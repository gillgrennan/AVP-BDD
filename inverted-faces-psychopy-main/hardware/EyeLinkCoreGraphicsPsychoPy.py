...
class EyeLinkCoreGraphicsPsychoPy(pylink.EyeLinkCustomDisplay):
    def __init__(self, tracker, win):
        ...
        self._version = '2021.11.JPDF'
        self._last_updated = '11/17/2021'
        ...
        if not DISABLE_AUDIO:
            try:
                self._target_beep = Sound('stimuli/type.wav', stereo=True)
                self._error_beep = Sound('stimuli/error.wav', stereo=True)
                self._done_beep = Sound('stimuli/qbeep.wav', stereo=True)
